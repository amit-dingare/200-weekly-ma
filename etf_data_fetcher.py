"""Fetch ETF data and calculate 200-week SMA proximity with RSI ~40."""
...
def calculate_200week_sma_etf(ticker, expense_ratio):
    """
    Calculate 200-week simple moving average, proximity, and 21-week RSI for an ETF.
    ...
    Returns:
        dict: Dictionary containing ticker, current_price, 52_week_low, sma_200, proximity, direction,
              rsi (21-week), rsi_proximity (distance from RSI target of 40), and expense_ratio
    ...
    for attempt in range(config.MAX_RETRIES):
        try:
            ...
            hist = etf.history(period="10y", interval=config.DATA_INTERVAL)
            ...
            daily_hist = etf.history(period="5d", interval="1d")
            ...
            daily_hist_1y = etf.history(period="1y", interval="1d")
            ...
            # Calculate proximity to RSI target (40 for ETFs instead of 20 for stocks)
            rsi_target = getattr(config, 'RSI_TARGET_ETF', 40)
            rsi_proximity = abs(rsi - rsi_target)
...
def get_top_etfs_near_sma(etf_expense_dict, top_n=5):
    """
    Get top N ETFs sorted by their proximity to 200-week SMA, with RSI proximity to 40 as tiebreaker.
    Fetches options data for top TOP_N_OPTIONS ETFs only.

    Args:
        etf_expense_dict (dict): Dictionary mapping ticker symbols to expense ratios
        top_n (int): Number of top ETFs to return

    Returns:
        pd.DataFrame: DataFrame with top N ETFs sorted by:
                      1) Proximity % to 200-week SMA (primary - most negative to most positive)
                      2) Proximity to RSI target of 40 (secondary/tiebreaker - closest to 40 first)
                      Includes options data for top TOP_N_OPTIONS ETFs
    """
    results = []
    tickers = list(etf_expense_dict.keys())
    total = len(tickers)

    print(f"Processing {total} ETFs...")

    for i, ticker in enumerate(tickers, 1):
        if i % 10 == 0:
            print(f"Progress: {i}/{total} ETFs processed")

        expense_ratio = etf_expense_dict[ticker]
        result = calculate_200week_sma_etf(ticker, expense_ratio)
        if result:
    ...
    print(f"\nSuccessfully processed {len(results)} out of {total} ETFs")
    ...
    # Sort by:
    # 1) Proximity % to 200-week SMA (primary - most negative to most positive)
    # 2) RSI proximity to target of 40 (secondary - tiebreaker, closest to 40 first)
    df_sorted = df.sort_values(['proximity_pct', 'rsi_proximity'], ascending=[True, True])

    # Fetch options data for top TOP_N_OPTIONS ETFs
    top_n_options = min(config.TOP_N_OPTIONS, len(df_sorted))
    if top_n_options > 0:
        print(f"\nFetching options data for top {top_n_options} ETFs...")
        ...
        print(f"Options data fetched for {top_n_options} ETFs")
...
if __name__ == "__main__":
    ...
    print("Testing with sample ETFs...")
    results = get_top_etfs_near_sma(test_etfs, top_n=5)
    print("\nResults:")
    print(results)
